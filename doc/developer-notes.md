...
- [Development guidelines](#development-guidelines)
    - [General VoidCoin Core](#general-voidcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
VoidCoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while voidcoind is running with the `logging`
RPC.  For instance, launching voidcoind with `-debug` or `-debug=1` will turn on
...
with "play voidcoins" on a test network.
...
VoidCoin Core is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at voidcoind startup or
`voidcoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `voidcoin-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/voidcoin/voidcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_voidcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_voidcoin --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/voidcoind -printtoconsole
...
# A coverage report will now be accessible at `./build/test_voidcoin.coverage/index.html`,
...
To profile a running voidcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep voidcoind` -- sleep 60
```
...
VoidCoin Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`voidcoind`)](https://doxygen.bitcoincore.org/voidcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `voidcoind.cpp`. Responsible for starting up and
...
- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.bitcoincore.org/httpserver_8cpp.html#aa6a7void_coin7265043bc0193220c5ae3a55f)
...
However, in open source software such as VoidCoin Core, where everyone uses
...
pay attention to for reviewers of VoidCoin Core code.

General VoidCoin Core
----------------------
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: VoidCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `VOIDCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef VOIDCOIN_FOO_BAR_H
#define VOIDCOIN_FOO_BAR_H
...
#endif // VOIDCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of VoidCoin Core, in which case
...
a VoidCoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/voidcoin-core/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/voidcoin-core/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/voidcoin-core/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/voidcoin-core/ctaes ; maintained by Core contributors.

- src/minisketch
  - Upstream at https://github.com/voidcoin-core/minisketch ; maintained by Core contributors.
...
file descriptors, it will cause problems with VoidCoin's `select()` loop, because
...
compatibility between nodes. This happened in VoidCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to VoidCoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/voidcoin/voidcoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/voidcoin/voidcoin/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/voidcoin/voidcoin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `voidcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `voidcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of VoidCoin Core, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/voidcoin/voidcoin/pull/14635).
