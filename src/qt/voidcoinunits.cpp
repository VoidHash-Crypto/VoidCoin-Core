...
static constexpr auto MAX_DIGITS_VOID = 16;

VoidCoinUnits::VoidCoinUnits(QObject *parent):
...
QString VoidCoinUnits::format(Unit unit, const CAmount& nIn, bool fPlus, SeparatorStyle separators, bool justify)
{
    ...
    if (justify) {
        quotient_str = quotient_str.rightJustified(MAX_DIGITS_VOID - num_decimals, ' ');
    }
...
bool VoidCoinUnits::parse(Unit unit, const QString& value, CAmount* val_out)
{
    if (value.isEmpty()) {
...
QString VoidCoinUnits::getAmountColumnTitle(Unit unit)
{
    return QObject::tr("Amount") + " (" + shortName(unit) + ")";
}

int VoidCoinUnits::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return unitlist.size();
}

QVariant VoidCoinUnits::data(const QModelIndex &index, int role) const
{
    int row = index.row();
...
CAmount VoidCoinUnits::maxMoney()
{
    return MAX_MONEY;
}
...
QDataStream& operator<<(QDataStream& out, const VoidCoinUnit& unit)
{
    return out << ToQint8(unit);
}

QDataStream& operator>>(QDataStream& in, VoidCoinUnit& unit)
{
    qint8 input;
    in >> input;
