...
int crypto_sign_keypair_from_seed(uint8_t *pk, uint8_t *sk, const uint8_t seed[SEEDBYTES]) {
  ...
  /*
   * Deterministic key generation from a caller-provided 32-byte seed.
   *
   * This mirrors crypto_sign_keypair(), but replaces randombytes(seedbuf,
   * SEEDBYTES) with the supplied seed. Keep the K/L domain bytes exactly
   * as upstream does before expanding rho/rhoprime/key.
   */
