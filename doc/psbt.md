# PSBT Howto for VoidCoin Core

Since VoidCoin Core 0.17, an RPC interface exists for Partially Signed VoidCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/voidcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for VoidCoin transactions that are not fully signed
...
#### Multisig with multiple VoidCoin Core instances
...
VoidCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `voidcoin-cli` is used.
...
Later, when *V* VOID has been received on *Amulti*, and Bob and Carol want to
