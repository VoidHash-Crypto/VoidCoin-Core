...
static void SetupVoidCoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " voidcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The voidcoin-tx tool is used for creating and modifying voidcoin transactions.\n\n"
                "voidcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded voidcoin transaction, or with \"-create [commands]\" to create a hex-encoded voidcoin transaction.\n"
                "\n"
                "Usage: voidcoin-tx [options] <hex-tx> [commands]\n"
                "or:    voidcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded voidcoin transaction
