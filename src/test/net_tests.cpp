...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2)
{
    ...
    s << "02"                                    // network type (IPv6)
         "10"                                    // address length
         "fd6b88c08724ca978112ca1bbdcafac2"_hex; // address: 0xfd + sha256("voidcoin")[0:5] +
                                                 // sha256(name)[0:10]
    s >> ser_params(addr);
...
BOOST_AUTO_TEST_CASE(initial_advertise_from_version_message)
{
    ...
    // genesis timestamp is ancient. VoidCoin regtest uses a fresh genesis
...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // limit. VoidCoin intentionally raises transport limits for 32 MB
        // blocks, so use a payload above the VoidCoin transport ceiling.
