# CJDNS support in VoidCoin Core

It is possible to run VoidCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of VoidCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the VoidCoin network and individual nodes.
...
before it will work with your VoidCoin Core node. This is described in steps
...
## Run VoidCoin Core with CJDNS

Once you are connected to the CJDNS network, the following VoidCoin Core
...
When enabled, this option tells VoidCoin Core that it is running in an
...
IPv6 local network. This helps VoidCoin Core perform better address management:
...
CJDNS support was added to VoidCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `voidcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/voidcoin/voidcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in VoidCoin Core

There are several ways to see your CJDNS address in VoidCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `voidcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `voidcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `voidcoin-cli help getnodeaddresses` for details.
