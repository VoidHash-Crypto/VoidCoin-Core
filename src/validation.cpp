...
static bool AllOutputsNativeP2QR(const CTransaction& tx)
{
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (!consensus::voidcoinp2qr::IsOutput(txout.scriptPubKey, ignored_program)) {
            return false;
        }
    }
...
static bool HasVoidCoinP2QRInput(const CTransaction& tx, const CCoinsViewCache& view)
{
    for (const CTxIn& txin : tx.vin) {
        ...
        if (consensus::voidcoinp2qr::IsOutput(coin.out.scriptPubKey, ignored_program) ||
...
static bool OversizedTxAllowedByP2QRPolicy(const CTransaction& tx, const CCoinsViewCache& view)
{
    ...
    if (!HasVoidCoinP2QRInput(tx, view)) {
        return false;
    }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Rather not work on nonstandard transactions. Native VOIDCOIN_P2QR spends are
    ...
    if (m_pool.m_opts.require_standard &&
        ...
        if (reason == "p2qr-scriptsig-size-deferred") {
            fDeferredP2QRScriptSigSize = true;
             ...
             LogPrintf("VOIDCOIN_P2QR policy debug: deferring P2QR scriptSig-size standardness check until inputs are available: tx=%s\n",
                tx.GetHash().ToString());
        } else if (reason == "p2qr-tx-size-deferred") {
            fDeferredP2QRTxSize = true;

            LogPrintf("VOIDCOIN_P2QR policy debug: deferring tx-size standardness check until inputs are available: tx=%s weight=%u\n",
    ...
    // That is unacceptable for VoidCoin wallet/exchange/pool safety.
    for (const CTxIn& txin : tx.vin) {
    ...
    if (fDeferredP2QRTxSize) {
        ...
        LogPrintf("VOIDCOIN_P2QR policy debug: accepted oversized P2QR tx-size exception: tx=%s weight=%u\n",
            tx.GetHash().ToString(), GetTransactionWeight(tx));
    }

    // Native and P2SH-carried VOIDCOIN_P2QR spends carry large ML-DSA-87 spend
    ...
    if (m_pool.m_opts.require_standard && (fDeferredP2QRScriptSigSize || fDeferredP2QRTxSize)) {
        for (const CTxIn& txin : tx.vin) {
            ...
            const bool is_native_p2qr =
                consensus::voidcoinp2qr::IsOutput(coin.out.scriptPubKey, program);

            const bool is_p2sh_carried_p2qr =
                !is_native_p2qr &&
                consensus::voidcoinp2qr::IsP2SHOutput(coin.out.scriptPubKey, script_id);
            ...
            while (pc < txin.scriptSig.end()) {
                ...
                if (!txin.scriptSig.GetOp(pc, opcode, data)) {
                    return state.Invalid(
                        ...
                        is_native_p2qr ? "void_coin-p2qr-scriptsig-format" : "void_coin-p2qr-p2sh-scriptsig-format",
                        "failed to parse VOIDCOIN_P2QR scriptSig pushes");
                }
            ...
            if (is_native_p2qr) {
                ...
                if (pushes.size() == 2) {
                    ...
                    if (!consensus::voidcoinp2qr::ParseSpendData(native_spend_script, sig, pubkey, p2qr_error)) {
                        return state.Invalid(
                            TxValidationResult::TX_NOT_STANDARD,
                            "void_coin-p2qr-scriptsig-format",
                            p2qr_error);
                    }
                } else {
                ...
                LogPrintf("VOIDCOIN_P2QR policy debug: accepted native P2QR oversized scriptSig envelope for prevout=%s:%u\n",
                ...
                if (pushes.size() < 3) {
                    return state.Invalid(
                        TxValidationResult::TX_NOT_STANDARD,
                        "void_coin-p2qr-p2sh-scriptsig-format",
                        strprintf("expected at least 3 pushes for P2SH-carried VOIDCOIN_P2QR spend, got %u", pushes.size()));
                }
                ...
                if (!consensus::voidcoinp2qr::IsOutput(redeem_script, redeem_program)) {
                    return state.Invalid(
                        TxValidationResult::TX_NOT_STANDARD,
                        "void_coin-p2qr-p2sh-redeemscript",
                        "P2SH-carried VOIDCOIN_P2QR redeemScript is not native VOIDCOIN_P2QR");
                }

                if (CScriptID(redeem_script) != script_id) {
                    return state.Invalid(
                        TxValidationResult::TX_NOT_STANDARD,
                        "void_coin-p2qr-p2sh-redeemscript-hash",
                        "P2SH-carried VOIDCOIN_P2QR redeemScript hash does not match prevout");
                }
                ...
                LogPrintf("VOIDCOIN_P2QR policy debug: accepted P2SH-carried P2QR oversized scriptSig envelope for prevout=%s:%u\n",
                    txin.prevout.hash.ToString(), txin.prevout.n);
            }
        }
    }
    ...
    // and P2SH-carried VOIDCOIN_P2QR signature envelopes are not priced like normal
    // transaction bytes.
    ...
    if (auto ancestors{m_subpackage.m_changeset->CalculateMemPoolAncestors(ws.m_tx_handle, maybe_rbf_limits)}) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/voidcoin-dev/2018-November/016518.html
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (tip && tip->nHeight + 1 >= m_active_chainstate.m_chainman.GetConsensus().nP2QRActivationHeight) {
        scriptVerifyFlags |= SCRIPT_VERIFY_VOIDCOIN_P2QR;
    }
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
     ...
     *
     * Height 2 onward:
     *   Normal mining subsidy begins at 50 VOID.
     *
     * This preserves:
     *   210,000,000 VOID mineable emission
    ...
    // VoidCoin normal mining subsidy halves every 2,100,000 one-minute blocks.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    ScriptError error{SCRIPT_ERR_UNKNOWN_ERROR};
        ...
        uint256 voidcoin_p2qr_program;
    if (consensus::voidcoinp2qr::IsOutput(m_tx_out.scriptPubKey, voidcoin_p2qr_program)) {
        std::string p2qr_error;

        if (!(nFlags & SCRIPT_VERIFY_VOIDCOIN_P2QR)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, native VOIDCOIN_P2QR inactive",
                nIn,
        ...
        if (!txdata || !txdata->m_spent_outputs_ready) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, missing spent outputs for native VOIDCOIN_P2QR validation",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
        ...
        if (!consensus::voidcoinp2qr::CheckSpend(*ptxTo, nIn, m_tx_out, txdata->m_spent_outputs, p2qr_error)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, native VOIDCOIN_P2QR validation failed: %s",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
    ...
    if (consensus::voidcoinp2qr::IsP2SHWrappedSpendCandidate(m_tx_out.scriptPubKey, scriptSig)) {
        std::string p2qr_error;

        if (!(nFlags & SCRIPT_VERIFY_VOIDCOIN_P2QR)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, P2SH-carried VOIDCOIN_P2QR inactive",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
        ...
        if (!txdata || !txdata->m_spent_outputs_ready) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, missing spent outputs for P2SH-carried VOIDCOIN_P2QR validation",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
        ...
        if (!consensus::voidcoinp2qr::CheckSpendP2SHWrapped(*ptxTo, nIn, m_tx_out, txdata->m_spent_outputs, p2qr_error)) {
            auto debug_str = strprintf(
                "input %i of %s (wtxid %s), spending %s:%i, P2SH-carried VOIDCOIN_P2QR validation failed: %s",
                nIn,
                ptxTo->GetHash().ToString(),
                ptxTo->GetWitnessHash().ToString(),
                ptxTo->vin[nIn].prevout.hash.ToString(),
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // See https://github.com/voidcoin/voidcoin/issues/22596 for additional information.
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    flags |= SCRIPT_VERIFY_VOIDCOIN_P2QR;
    }
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // in "Weaknesses in VoidCoin’s Merkle Root Construction":
        // https://lists.linuxfoundation.org/pipermail/voidcoin-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // blocks to prevent timewarp attacks (see https://github.com/voidcoin/voidcoin/pull/15482).
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    /*
     * VoidCoin dev fund creation consensus rule:
     *
     * Block 1 must be the deterministic dev fund block.
     *
     * It must contain exactly one transaction: the coinbase.
     * That coinbase must create exactly 21 outputs of 1,000,000 VOID each,
    ...
    /*
     * VoidCoin disables SegWit/Taproot/witness paths.
     *
     ...
     * VoidCoin transaction output consensus rules.
     */
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/voidcoin-dev/2019-February/016697.html.  Because CheckBlock() is
