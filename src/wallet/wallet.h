...
#ifndef VOIDCOIN_WALLET_WALLET_H
#define VOIDCOIN_WALLET_WALLET_H
...
//
// 10000 quarks/kvB = 0.00010000 VOID/kvB.
...
struct VoidCoinVaultTxPackage
{
    CMutableTransaction tx;
    std::vector<VoidCoinVaultInput> inputs;
    std::vector<VoidCoinVaultApproval> approvals;

    std::string vault_label;
    VoidCoinP2QRDestination vault_dest;
...
public:
     ...
     * voidcoin-wallet tool code or by wallet migration code. If this is called
    ...
    /** Seed-backed native VOID P2QR keys, indexed by HASH256(ML-DSA-87 pubkey). */
    std::unique_ptr<VoidCoinP2QRScriptPubKeyMan> m_voidcoin_p2qr_spk_man GUARDED_BY(cs_wallet);
    
    
    std::optional<VoidCoinP2QRSpendInfo> GetVoidCoinP2QRSpendInfoForScript(const CScript& script) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    
    bool LoadVoidCoinP2QRMultisig(const uint256& program, const VoidCoinP2QRMultisigRecord& record) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<VoidCoinP2QRDestination> AddVoidCoinP2QRMultisig(
       uint16_t required,
       std::vector<uint256> signer_programs,
       const std::string& label) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    
    util::Result<VoidCoinVaultTxPackage> CreateVoidCoinVaultTxPackage(
        const std::vector<CRecipient>& recipients,
        const CCoinControl& coin_control) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<VoidCoinVaultTxPackage> ApproveVoidCoinVaultTxPackage(
        const VoidCoinVaultTxPackage& package) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<CTransactionRef> FinalizeVoidCoinVaultTxPackage(
        const VoidCoinVaultTxPackage& package) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
        
    util::Result<std::string> EncodeVoidCoinVaultTxPackage(
        const VoidCoinVaultTxPackage& package) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    util::Result<VoidCoinVaultTxPackage> DecodeVoidCoinVaultTxPackage(
    ...
    util::Result<CScript> GetNewVoidCoinP2QRChangeScript();
    util::Result<CScript> GetNewVoidCoinP2QRCompatibilityChangeScript();
    util::Result<VoidCoinP2QRDestination> GenerateNewVoidCoinP2QRDestination(const std::string& label);
    bool ImportVoidCoinP2QRSeed(Span<const unsigned char> seed, const std::string& label, VoidCoinP2QRDestination& dest) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool GetVoidCoinP2QRSeed(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& seed_out) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool GetVoidCoinP2QRKey(const VoidCoinP2QRDestination& dest, CPQKey& key_out) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool HaveVoidCoinP2QRDestination(const VoidCoinP2QRDestination& dest) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool GetVoidCoinP2QRPubKey(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& pubkey_out) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    DBErrors LoadWallet();
    bool LoadVoidCoinP2QRKey(const uint256& key_hash, const P2QRKeyRecord& record) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    //! VoidCoinP2QR Script PubKeyMan.
    VoidCoinP2QRScriptPubKeyMan* GetVoidCoinP2QRScriptPubKeyMan() const;
    VoidCoinP2QRScriptPubKeyMan* GetOrCreateVoidCoinP2QRScriptPubKeyMan();
    bool GetVoidCoinP2QRDestinationForScript(const CScript& script, VoidCoinP2QRDestination& dest) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
...
#endif // VOIDCOIN_WALLET_WALLET_H
