...
Below are some notes on how to build VoidCoin Core for Windows.

The options known to work for building VoidCoin Core on Windows are:
...
Note that for WSL the VoidCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/voidcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\voidcoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/voidcoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/voidcoin --strip
```
