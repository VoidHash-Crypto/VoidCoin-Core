...
static CScript VoidCoinTestP2QRScriptForTxTests(uint8_t tag = 0xC5)
{
...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    // validity vectors. VoidCoin intentionally rejects those as normal
    // transaction outputs/spends. Keep this test as a VoidCoin-native transaction
    ...
    tx.vout[0].scriptPubKey = VoidCoinTestP2QRScriptForTxTests();
    ...
    BOOST_CHECK(tx_const.vout[0].scriptPubKey == VoidCoinTestP2QRScriptForTxTests());
}

BOOST_AUTO_TEST_CASE(tx_invalid)
{
    // Bitcoin's tx_invalid.json is a legacy Bitcoin script fixture set.
    // VoidCoin deliberately changed the valid transaction/output universe, so
...

    {
        ...
        tx.vout.resize(1);
        tx.vout[0].nValue = COIN;
        tx.vout[0].scriptPubKey = VoidCoinTestP2QRScriptForTxTests(0xD0);
...

    {
        ...
        tx.vout.resize(1);
        tx.vout[0].nValue = -1;
        tx.vout[0].scriptPubKey = VoidCoinTestP2QRScriptForTxTests(0xD1);
...

    {
        ...
        tx.vout[0].scriptPubKey = VoidCoinTestP2QRScriptForTxTests(0xD2);
...

    {
        ...
        tx.vout.resize(1);
        tx.vout[0].nValue = COIN;
        tx.vout[0].scriptPubKey = VoidCoinTestP2QRScriptForTxTests(0xD3);
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    // legacy transaction and expects it to be valid. VoidCoin's valid output
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = COIN;
    tx.vout[0].scriptPubKey = VoidCoinTestP2QRScriptForTxTests(0xE0);

    TxValidationState state;
    BOOST_CHECK_MESSAGE(CheckTransaction(CTransaction{tx}, state), "VoidCoin-native transaction should be structurally valid");
}
