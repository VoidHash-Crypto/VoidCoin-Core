...
Tracing (USDT) support for VoidCoin Core on Linux using. For more information on
USDT support in VoidCoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `voidcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `voidcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[VoidCoin Core USDT documentation] on how to list available tracepoints in your
`voidcoind` binary.

[VoidCoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof voidcoind)
```
...
VoidCoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof voidcoind)
```
...
In a different terminal, starting VoidCoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/voidcoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof voidcoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof voidcoind)
```
