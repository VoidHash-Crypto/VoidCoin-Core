...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose, bool rescanning_old_block)
{
    ...
    if (!strCmd.empty())
    {
        ...
        // https://github.com/voidcoin/voidcoin/pull/13339#issuecomment-537384875
        // A few ways it could be implemented in the future are described in:
        // https://github.com/voidcoin/voidcoin/pull/13339#issuecomment-461288094
...
bool CWallet::LoadToWallet(const uint256& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    // If wallet doesn't have a chain (e.g when using voidcoin-wallet tool),
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        //    https://github.com/voidcoin/voidcoin/pull/18600#discussion_r420195993.
        ...
        //    https://github.com/voidcoin/voidcoin/pull/9371 and the prior sync
        ...
        // https://github.com/voidcoin-core/voidcoin-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
...
isminetype CWallet::IsMine(const CScript& script) const
{
    ...
    // Native / P2SH-carried VOID P2QR wallet.
    //
    // Keep this ahead of m_cached_spks. VoidCoin P2QR ownership is handled by
    // the wallet-native P2QR SPKM, including multisig policy records that are
    // not normal Bitcoin descriptor scripts.
    if (VoidCoinP2QRScriptPubKeyMan* spkm = GetVoidCoinP2QRScriptPubKeyMan()) {
...
bool CWallet::LoadVoidCoinP2QRKey(const uint256& key_hash, const P2QRKeyRecord& record)
{
    AssertLockHeld(cs_wallet);
    return GetOrCreateVoidCoinP2QRScriptPubKeyMan()->LoadKey(key_hash, record);
}
...
bool CWallet::SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors) const
{
    ...
    VoidCoinP2QRScriptPubKeyMan* p2qr_spkm{nullptr};
    ...
    for (uint32_t i = 0; i < tx.vin.size(); ++i) {
        ...
        // Native/P2SH-carried VOIDCOIN_P2QR has a dedicated signer.
        if (GetVoidCoinP2QRSpendInfoForScript(script_pubkey)) {
            VoidCoinP2QRScriptPubKeyMan* qr = GetVoidCoinP2QRScriptPubKeyMan();
            if (!qr) {
                input_errors[i] = Untranslated("VOIDCOIN_P2QR signer not available");
                return false;
            }
    ...
    // inputs they do not own. Run them before VOIDCOIN_P2QR so they cannot clobber the
    ...
    // Then sign VOIDCOIN_P2QR last. The P2QR scriptSig envelope must be final:
    //
    //   native:       <sig> <pubkey>
    ...
    for (uint32_t i = 0; i < signed_tx.vin.size(); ++i) {
        ...
        if (consensus::voidcoinp2qr::IsOutput(spent_output.scriptPubKey, ignored_program)) {
            if (!consensus::voidcoinp2qr::CheckSpend(signed_tx, i, spent_output, spent_outputs, verify_error)) {
                input_errors[i] = Untranslated(strprintf("VOIDCOIN_P2QR self-verification failed: %s", verify_error));
                complete = false;
            }
            continue;
        }

        if (consensus::voidcoinp2qr::IsP2SHWrappedSpendCandidate(spent_output.scriptPubKey, txin.scriptSig)) {
            if (!consensus::voidcoinp2qr::CheckSpendP2SHWrapped(signed_tx, i, spent_output, spent_outputs, verify_error)) {
                input_errors[i] = Untranslated(strprintf("VOIDCOIN_P2QR P2SH self-verification failed: %s", verify_error));
                complete = false;
            }
            continue;
        }
...
util::Result<VoidCoinP2QRDestination> CWallet::GenerateNewVoidCoinP2QRDestination(const std::string& label)
{
    LOCK(cs_wallet);

    auto* spk_man = GetOrCreateVoidCoinP2QRScriptPubKeyMan();
    ...
    const auto* qr_dest = std::get_if<VoidCoinP2QRDestination>(&*op_dest);
    if (!qr_dest) {
        return util::Error{_("Error: VOID P2QR SPKM returned non-P2QR destination.")};
    }

    WalletBatch batch(GetDatabase());
    if (!SetAddressBookWithDB(batch, *qr_dest, label, AddressPurpose::RECEIVE)) {
        return util::Error{_("Error: Failed to add VOID P2QR address to address book.")};
    }

    return *qr_dest;
}

util::Result<CScript> CWallet::GetNewVoidCoinP2QRChangeScript()
{
    auto op_dest = GenerateNewVoidCoinP2QRDestination("");
    if (!op_dest) {
...
util::Result<CScript> CWallet::GetNewVoidCoinP2QRCompatibilityChangeScript()
{
    auto op_dest = GenerateNewVoidCoinP2QRDestination("");
    if (!op_dest) {
        return util::Error{util::ErrorString(op_dest)};
...
bool CWallet::GetVoidCoinP2QRDestinationForScript(const CScript& script, VoidCoinP2QRDestination& dest) const
{
    AssertLockHeld(cs_wallet);

    uint256 program;
    if (consensus::voidcoinp2qr::IsOutput(script, program)) {
        VoidCoinP2QRDestination candidate{program};
        if (HaveVoidCoinP2QRDestination(candidate)) {
            dest = candidate;
            return true;
        }
        return false;
    ...
    CScriptID script_id;
    if (!consensus::voidcoinp2qr::IsP2SHOutput(script, script_id)) {
        return false;
    }

    const VoidCoinP2QRScriptPubKeyMan* qr_spkm = GetVoidCoinP2QRScriptPubKeyMan();
    if (!qr_spkm) {
        return false;
    }
...
bool CWallet::ImportVoidCoinP2QRSeed(Span<const unsigned char> seed, const std::string& label, VoidCoinP2QRDestination& dest)
{
    AssertLockHeld(cs_wallet);
...
bool CWallet::GetVoidCoinP2QRSeed(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& seed_out) const
{
    AssertLockHeld(cs_wallet);

    auto* spk_man = GetVoidCoinP2QRScriptPubKeyMan();
    return spk_man && spk_man->ExportSeed(dest, seed_out);
}

bool CWallet::GetVoidCoinP2QRKey(const VoidCoinP2QRDestination& dest, CPQKey& key_out) const
{
    AssertLockHeld(cs_wallet);
...
bool CWallet::HaveVoidCoinP2QRDestination(const VoidCoinP2QRDestination& dest) const
{
    AssertLockHeld(cs_wallet);
...
bool CWallet::GetVoidCoinP2QRPubKey(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& pubkey_out) const
{
    AssertLockHeld(cs_wallet);
...
static std::vector<unsigned char> VoidCoinP2QRUInt16Push(uint16_t value)
{
    /*
     * VoidCoin P2QR multisig signer indexes / threshold values are parsed as
...
bool CWallet::LoadVoidCoinP2QRMultisig(const uint256& program, const VoidCoinP2QRMultisigRecord& record)
{
    AssertLockHeld(cs_wallet);

    VoidCoinP2QRScriptPubKeyMan* qr_spkm = GetVoidCoinP2QRScriptPubKeyMan();
    if (!qr_spkm) return false;

    return qr_spkm->LoadMultisigPolicy(program, record);
}

util::Result<VoidCoinP2QRDestination> CWallet::AddVoidCoinP2QRMultisig(
    uint16_t required,
    ...
    const std::string& label)
{
    ...
    std::string error;
    if (!VoidCoinIsCanonicalP2QRMultisigPolicy(required, signer_programs, &error)) {
        return util::Error{Untranslated(strprintf("Invalid VOIDCOIN_P2QR multisig policy: %s", error))};
    }

    const uint256 program = VoidCoinP2QRMultisigProgram(required, signer_programs);

    VoidCoinP2QRMultisigRecord record;
    record.required = required;
    record.signer_programs = signer_programs;

    VoidCoinP2QRScriptPubKeyMan* qr_spkm = GetVoidCoinP2QRScriptPubKeyMan();
    if (!qr_spkm) {
        return util::Error{Untranslated("VOIDCOIN_P2QR ScriptPubKeyMan not available")};
    }

    if (!qr_spkm->AddMultisigPolicy(program, record)) {
        return util::Error{Untranslated("Failed to store VOIDCOIN_P2QR multisig policy")};
    }

    const VoidCoinP2QRDestination dest{program};
...
static void AddOrReplaceVoidCoinVaultApproval(
    std::vector<VoidCoinVaultApproval>& approvals,
    VoidCoinVaultApproval approval)
{
    for (VoidCoinVaultApproval& existing : approvals) {
...
util::Result<VoidCoinVaultTxPackage> CWallet::ApproveVoidCoinVaultTxPackage(
    const VoidCoinVaultTxPackage& package)
{
    VoidCoinVaultTxPackage out = package;
    ...
    if (out.required == 0 || out.signer_programs.empty() || out.required > out.signer_programs.size()) {
        return util::Error{Untranslated("invalid VoidCoin vault package threshold policy")};
    }
     ...
     */
    const uint256 expected_program = VoidCoinP2QRMultisigProgram(out.required, out.signer_programs);
    if (expected_program != out.vault_program) {
        return util::Error{Untranslated(
            "invalid VoidCoin vault package: vault program does not match signer address threshold policy")};
    }
    ...
    if (!qr_spkm->HaveMultisigProgram(out.vault_program)) {
        ...
        if (!qr_spkm->AddMultisigPolicy(out.vault_program, record)) {
            return util::Error{Untranslated(
                "failed to import VoidCoin vault policy into this wallet")};
        }
    ...
    for (const VoidCoinVaultInput& input : out.inputs) {
    ...
    for (uint32_t input_index = 0; input_index < out.tx.vin.size(); ++input_index) {
        ...
        if (consensus::voidcoinp2qr::IsOutput(spent_output.scriptPubKey, program)) {
            wrapped_p2sh = false;
            redeem_script = consensus::voidcoinp2qr::MakeRedeemScript(program);
        } else {
            CScriptID script_id;
            if (!consensus::voidcoinp2qr::IsP2SHOutput(spent_output.scriptPubKey, script_id)) {
                continue;
            }
        ...
        const uint256 msg_hash = !wrapped_p2sh
            ? consensus::voidcoinp2qr::SignatureHash(
                  tx_const_unsigned,
                  input_index,
                  spent_output,
                  spent_outputs)
            : consensus::voidcoinp2qr::SignatureHashP2SHWrapped(
                  tx_const_unsigned,
                  input_index,
                  spent_output,
                  spent_outputs);
        ...
        for (uint16_t signer_index = 0;
            ...
            if (derived_signer_program != signer_program) {
                return util::Error{Untranslated(
                    "VOIDCOIN_P2QR local signer pubkey does not match signer address policy")};
            }
            ...
            if (!key.Sign(Span<const unsigned char>(msg_hash.begin(), msg_hash.size()), signature)) {
                return util::Error{Untranslated("VOIDCOIN_P2QR vault approval signing failed")};
            }

            if (signature.empty()) {
                return util::Error{Untranslated("VOIDCOIN_P2QR vault approval signing returned empty signature")};
            }
            ...
            AddOrReplaceVoidCoinVaultApproval(out.approvals, std::move(approval));
    ...
    for (uint32_t input_index = 0; input_index < out.tx.vin.size(); ++input_index) {
        ...
        for (const VoidCoinVaultApproval& approval : out.approvals) {
...
util::Result<CTransactionRef> CWallet::FinalizeVoidCoinVaultTxPackage(
    const VoidCoinVaultTxPackage& package)
{
    ...
    for (const VoidCoinVaultInput& input : package.inputs) {
        spent_outputs.emplace_back(input.amount, input.script_pub_key);
    }
    ...
    for (uint32_t input_index = 0; input_index < mtx.vin.size(); ++input_index) {
        ...
        const auto spend_info = GetVoidCoinP2QRSpendInfoForScript(spent_output.scriptPubKey);
        if (!spend_info || !spend_info->multisig) {
            continue;
        }

        std::vector<VoidCoinVaultApproval> input_approvals;
        for (const VoidCoinVaultApproval& approval : package.approvals) {
            if (approval.input_index == input_index) {
        ...
        std::sort(input_approvals.begin(), input_approvals.end(),
            [](const VoidCoinVaultApproval& a, const VoidCoinVaultApproval& b) {
        ...
        if (input_approvals.size() < spend_info->multisig_required) {
            return util::Error{Untranslated(strprintf(
                "VOIDCOIN_P2QR vault package is incomplete: input %u has %u approvals, needs %u",
                input_index,
        ...
        if (spend_info->multisig_signer_programs.empty() ||
            ...
            return util::Error{Untranslated(
                "VOIDCOIN_P2QR vault spend info has invalid signer address policy")};
        }
        ...
        for (const VoidCoinVaultApproval& approval : input_approvals) {
            if (added >= spend_info->multisig_required) {
                break;
            }

            if (approval.signer_index >= spend_info->multisig_signer_programs.size()) {
                return util::Error{Untranslated("VOIDCOIN_P2QR vault approval has invalid signer index")};
            }

            const uint256 derived_signer_program = VoidCoinP2QRSingleProgram(approval.pubkey);
            if (derived_signer_program != spend_info->multisig_signer_programs[approval.signer_index]) {
                return util::Error{Untranslated(
                    "VOIDCOIN_P2QR vault approval pubkey does not match signer address")};
            }
        ...
        if (added < spend_info->multisig_required) {
            return util::Error{Untranslated(strprintf(
                "VOIDCOIN_P2QR vault package is incomplete: input %u has %u usable approvals, needs %u",
                input_index,
        ...
        for (const uint16_t signer_index : signer_indexes) {
            script_sig << VoidCoinP2QRUInt16Push(signer_index);
        }
        ...
        script_sig << VoidCoinP2QRUInt16Push(spend_info->multisig_required);
        script_sig << VoidCoinP2QRUInt16Push(static_cast<uint16_t>(spend_info->multisig_signer_programs.size()));
    ...
    for (uint32_t input_index = 0; input_index < signed_tx.vin.size(); ++input_index) {
        ...
        const bool ok = !spend_info->wrapped_p2sh
            ? consensus::voidcoinp2qr::CheckSpend(
                  signed_tx,
                  input_index,
                  spent_output,
                  spent_outputs,
                  verify_error)
            : consensus::voidcoinp2qr::CheckSpendP2SHWrapped(
                  signed_tx,
                  input_index,
                  spent_output,
                  spent_outputs,
        ...
        if (!ok) {
            return util::Error{Untranslated(strprintf(
                "VOIDCOIN_P2QR vault final transaction verification failed: %s",
                verify_error))};
        }
    }
...
util::Result<VoidCoinVaultTxPackage> CWallet::CreateVoidCoinVaultTxPackage(
    ...
    const CCoinControl& coin_control)
{
    ...
    auto txr = CreateTransaction(
     ...
     VoidCoinVaultTxPackage package;
      ...
      */
      static constexpr uint32_t VOIDCOIN_VAULT_RBF_SEQUENCE = 0xfffffffd;

      for (CTxIn& txin : package.tx.vin) {
          txin.nSequence = VOIDCOIN_VAULT_RBF_SEQUENCE;
      }
    ...
    for (const CTxIn& txin : package.tx.vin) {
        ...
        const auto spend_info = GetVoidCoinP2QRSpendInfoForScript(prev_txout.scriptPubKey);
        if (!spend_info) {
            return util::Error{Untranslated(strprintf(
                "vault package input %s:%u is not a wallet-recognized VoidCoin P2QR output",
                prevout.hash.ToString(),
                prevout.n))};
        }
        ...
        if (!spend_info->multisig) {
            return util::Error{Untranslated(strprintf(
                "vault package input %s:%u is not a VoidCoin P2QR multisig vault output",
                prevout.hash.ToString(),
                prevout.n))};
        }
        ...
        if (consensus::voidcoinp2qr::IsOutput(prev_txout.scriptPubKey, actual_vault_program)) {
            // Native P2QR vault output: 5020 <32-byte program>
        } else if (consensus::voidcoinp2qr::IsP2SHOutput(prev_txout.scriptPubKey, actual_script_id)) {
            ...
            return util::Error{Untranslated(strprintf(
                "vault package input %s:%u is not a native or wrapped VoidCoin P2QR vault output",
                prevout.hash.ToString(),
                prevout.n))};
        }
        ...
        const uint256 expected_program = VoidCoinP2QRMultisigProgram(actual_required, actual_signer_programs);
        if (expected_program != actual_vault_program) {
            return util::Error{Untranslated(strprintf(
                "internal VoidCoin vault package error: selected vault output program %s does not match M-of-N signer address policy program %s",
        ...
        if (!found_vault_input) {
            ...
            package.vault_dest = VoidCoinP2QRDestination{actual_vault_program};
...

    {
        ...
        for (const VoidCoinVaultInput& input : package.inputs) {
            const auto spend_info = GetVoidCoinP2QRSpendInfoForScript(input.script_pub_key);
            if (!spend_info) {
                return util::Error{Untranslated(
    ...
    auto approved = ApproveVoidCoinVaultTxPackage(package);
    if (!approved) {
...
util::Result<std::string> CWallet::EncodeVoidCoinVaultTxPackage(
    const VoidCoinVaultTxPackage& package) const
{
    UniValue root(UniValue::VOBJ);

    root.pushKV("format", "voidcoin-vault-tx");
    ...
    for (const uint256& signer_program : package.signer_programs) {
        ...
        signer_addresses.push_back(EncodeDestination(VoidCoinP2QRDestination{signer_program}));
    }
...
util::Result<VoidCoinVaultTxPackage> CWallet::DecodeVoidCoinVaultTxPackage(
    const std::string& json) const
{
    UniValue root;
    if (!root.read(json)) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: JSON parse failed")};
    }

    if (!root.isObject()) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: root is not an object")};
    }

    if (!root.exists("format") || root["format"].get_str() != "voidcoin-vault-tx") {
        return util::Error{Untranslated("invalid VoidCoin vault payment file format")};
    }
    ...
    if (!vault.isObject()) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: missing vault object")};
    }

    package.vault_label = vault.exists("label") ? vault["label"].get_str() : "";

    if (!vault.exists("address")) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: missing vault address")};
    }

    const CTxDestination vault_dest = DecodeDestination(vault["address"].get_str());
    if (!std::holds_alternative<VoidCoinP2QRDestination>(vault_dest)) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: vault address is not P2QR")};
    }
    package.vault_dest = std::get<VoidCoinP2QRDestination>(vault_dest);

    if (!vault.exists("program") || !IsHex(vault["program"].get_str())) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: invalid vault program")};
    }

    const std::vector<unsigned char> program_bytes = ParseHex(vault["program"].get_str());
    if (program_bytes.size() != 32) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: vault program must be 32 bytes")};
    }
    std::copy(program_bytes.begin(), program_bytes.end(), package.vault_program.begin());

    if (!vault.exists("required")) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: missing required approvals")};
    }
    ...
    if (vault.exists("signer_programs")) {
        ...
        if (!signer_programs.isArray()) {
            return util::Error{Untranslated("invalid VoidCoin vault payment file: signer_programs is not an array")};
        }

        for (unsigned int i = 0; i < signer_programs.size(); ++i) {
            const std::string program_hex = signer_programs[i].get_str();
            if (!IsHex(program_hex)) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: invalid signer program hex")};
            }

            const std::vector<unsigned char> signer_program_bytes = ParseHex(program_hex);
            if (signer_program_bytes.size() != 32) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: signer program must be 32 bytes")};
            }
        ...
        if (!signer_addresses.isArray()) {
            return util::Error{Untranslated("invalid VoidCoin vault payment file: signer_addresses is not an array")};
        }
        ...
        for (unsigned int i = 0; i < signer_addresses.size(); ++i) {
            ...
            if (!std::holds_alternative<VoidCoinP2QRDestination>(signer_dest)) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: signer address is not P2QR")};
            }
        ...
        return util::Error{Untranslated(
            "invalid VoidCoin vault payment file: missing signer_programs array")};
    }

    std::string policy_error;
    if (!VoidCoinIsCanonicalP2QRMultisigPolicy(package.required, package.signer_programs, &policy_error)) {
        return util::Error{Untranslated(strprintf(
            "invalid VoidCoin vault payment file: invalid signer address policy: %s",
            policy_error))};
    }

    const uint256 expected_vault_program =
        VoidCoinP2QRMultisigProgram(package.required, package.signer_programs);
    if (expected_vault_program != package.vault_program) {
        return util::Error{Untranslated(
            "invalid VoidCoin vault payment file: vault program does not match signer address policy")};
    }

    const UniValue& tx_obj = root["transaction"];
    if (!tx_obj.isObject() || !tx_obj.exists("unsigned_hex")) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: missing transaction hex")};
    }

    const std::string tx_hex = tx_obj["unsigned_hex"].get_str();
    if (!IsHex(tx_hex)) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: invalid transaction hex")};
    }
    ...
    try {
        ...
        return util::Error{Untranslated(strprintf(
            "invalid VoidCoin vault payment file: transaction decode failed: %s",
            e.what()))};
    }
    ...
    if (!inputs.isArray()) {
        return util::Error{Untranslated("invalid VoidCoin vault payment file: missing inputs array")};
    }

    for (unsigned int i = 0; i < inputs.size(); ++i) {
        const UniValue& in = inputs[i];
        if (!in.isObject()) {
            return util::Error{Untranslated("invalid VoidCoin vault payment file: input is not an object")};
        }

        VoidCoinVaultInput input;

        if (!in.exists("txid") || !in.exists("vout") || !in.exists("amount") || !in.exists("scriptPubKey")) {
            return util::Error{Untranslated("invalid VoidCoin vault payment file: input is missing required fields")};
        }

        const auto txid = Txid::FromHex(in["txid"].get_str());
        if (!txid) {
            return util::Error{Untranslated("invalid VoidCoin vault payment file: invalid input txid")};
        }
        ...
        if (!IsHex(script_hex)) {
            return util::Error{Untranslated("invalid VoidCoin vault payment file: invalid input scriptPubKey hex")};
        }
    ...
    if (approvals.isArray()) {
        for (unsigned int i = 0; i < approvals.size(); ++i) {
            ...
            if (!a.isObject()) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: approval is not an object")};
            }

            if (!a.exists("input") || !a.exists("signer_index") || !a.exists("pubkey") || !a.exists("signature")) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: approval is missing required fields")};
            }

            const std::string pubkey_hex = a["pubkey"].get_str();
            if (!IsHex(pubkey_hex)) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: invalid approval pubkey hex")};
            }

            const std::vector<unsigned char> pubkey = ParseHex(pubkey_hex);
            if (pubkey.size() != VOIDCOIN_P2QR_PUBKEY_SIZE) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: approval pubkey has invalid size")};
            }

            const std::string signature_hex = a["signature"].get_str();
            if (!IsHex(signature_hex)) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: invalid approval signature hex")};
            }

            const std::vector<unsigned char> signature = ParseHex(signature_hex);
            if (signature.size() != VOIDCOIN_P2QR_SIGNATURE_SIZE) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: approval signature has invalid size")};
            }
            ...
            if (approval.signer_index >= package.signer_programs.size()) {
                return util::Error{Untranslated("invalid VoidCoin vault payment file: approval signer index is out of range")};
            }

            const uint256 derived_signer_program = VoidCoinP2QRSingleProgram(approval.pubkey);
            if (derived_signer_program != package.signer_programs[approval.signer_index]) {
                return util::Error{Untranslated(
                    "invalid VoidCoin vault payment file: approval pubkey does not match signer address")};
            }
 ...
 * https://voidcointalk.org/?topic=54527, or
 * https://github.com/voidcoin/voidcoin/pull/1393.
 */
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart voidcoind with -walletcrosschain to override.");
                return false;
            }
        }
    }
...
std::set<ScriptPubKeyMan*> CWallet::GetAllScriptPubKeyMans() const
{
    ...
    if (m_voidcoin_p2qr_spk_man) {
        spk_mans.insert(m_voidcoin_p2qr_spk_man.get());
    }
    return spk_mans;
}
...
std::set<ScriptPubKeyMan*> CWallet::GetScriptPubKeyMans(const CScript& script) const
{
    ...
    // Native VOID P2QR wallet
    if (VoidCoinP2QRScriptPubKeyMan* qr_spkm = GetVoidCoinP2QRScriptPubKeyMan()) {
...
VoidCoinP2QRScriptPubKeyMan* CWallet::GetVoidCoinP2QRScriptPubKeyMan() const
{
    AssertLockHeld(cs_wallet);
    return m_voidcoin_p2qr_spk_man.get();
}

VoidCoinP2QRScriptPubKeyMan* CWallet::GetOrCreateVoidCoinP2QRScriptPubKeyMan()
{
    AssertLockHeld(cs_wallet);
    if (!m_voidcoin_p2qr_spk_man) {
        m_voidcoin_p2qr_spk_man = std::make_unique<VoidCoinP2QRScriptPubKeyMan>(*this);
    }
    return m_voidcoin_p2qr_spk_man.get();
}
...
std::optional<VoidCoinP2QRSpendInfo> CWallet::GetVoidCoinP2QRSpendInfoForScript(const CScript& script) const
{
    AssertLockHeld(cs_wallet);
    ...
    if (consensus::voidcoinp2qr::IsOutput(script, program)) {
        ...
        if (qr_spkm->GetMultisigProgramForP2SHScriptID(script_id, resolved_program)) {
             ...
             */
            VoidCoinP2QRDestination resolved_dest;
    ...
    VoidCoinP2QRMultisigRecord multisig_record;
    bool is_multisig = false;

    if (qr_spkm->GetMultisigRecord(program, multisig_record)) {
        const uint256 expected_program = VoidCoinP2QRMultisigProgram(
        ...
        std::string policy_error;
        if (!VoidCoinIsCanonicalP2QRMultisigPolicy(
                multisig_record.required,
                multisig_record.signer_programs,
                &policy_error)) {
            return std::nullopt;
    ...
    if (!is_multisig && !HaveVoidCoinP2QRDestination(dest)) {
        return std::nullopt;
    }
