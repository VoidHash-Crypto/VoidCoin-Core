...
/** Call node.updateRwSetting() with VoidCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because VoidCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/voidcoin/voidcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because voidcoin 22.x will not try to read these.
...
//! Convert enabled/size values to voidcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from voidcoin -prune setting.
...
//! Get pruning size value to show in GUI from voidcoin -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if voidcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_voidcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<VoidCoinUnit>() == m_display_voidcoin_unit) return;
    m_display_voidcoin_unit = new_unit.value<VoidCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayVoidCoinUnit", QVariant::fromValue(m_display_voidcoin_unit));
    Q_EMIT displayUnitChanged(m_display_voidcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/voidcoin/voidcoin/pull/8273
    ...
    // (https://github.com/voidcoin-core/gui/issues/567).
