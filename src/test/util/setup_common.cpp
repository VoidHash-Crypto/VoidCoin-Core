...
constexpr inline auto TEST_DIR_PATH_ELEMENT{"test_common voidcoin"}; // Includes a space to catch possible path escape issues.
...
static CScript VoidCoinTestP2QRScript(unsigned char tag = 1)
{
    ...
    : TestingSetup{ChainType::REGTEST, opts}
{
    ...
    //
    // VoidCoin genesis is not Bitcoin's old test timestamp, so anchor mock time
    ...
    // accept CKey parameters. The fixture chain itself mines to a VoidCoin P2QR
...
{
    const CScript scriptPubKey = VoidCoinTestP2QRScript();
    ...
    for (int i = 0; i < num_blocks; ++i) {
        ...
        // Advance by VoidCoin's configured spacing. This keeps the fixture in
        // the same time domain as VoidCoin's DA instead of mining one-second
    ...
    Chainstate& chainstate)
{
    ...
    // select the expected VoidCoin difficulty bits. Validation will reject blocks
                                                                                  ...
                                                                                  const std::optional<uint32_t>& fee_output)
{
    ...
    // still exercise legacy script tests. VoidCoin-native P2QR transaction
...
std::vector<CTransactionRef> TestChain100Setup::PopulateMempool(FastRandomContext& det_rand, size_t num_transactions, bool submit)
{
    ...
    while (num_transactions > 0 && !unspent_prevouts.empty()) {
        ...
        for (size_t n{0}; n < num_outputs; ++n) {
            CScript spk = VoidCoinTestP2QRScript(static_cast<unsigned char>((num_transactions + n) & 0xff));
...
void TestChain100Setup::MockMempoolMinFee(const CFeeRate& target_feerate)
{
    ...
    mtx.vout.emplace_back(1 * COIN, VoidCoinTestP2QRScript(42));
 ...
 *
 * Do not use this as a VoidCoin consensus-valid block fixture.
 */
