...
const std::string VoidCoinGUI::DEFAULT_UIPLATFORM =
...
VoidCoinGUI::VoidCoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &VoidCoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &VoidCoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &VoidCoinGUI::showModalOverlay);
...
void VoidCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a VoidCoin address"));
    ...
    vaultsAction->setStatusTip(tr("Create, approve, and broadcast VoidCoin multi-signature vault payments"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and voidcoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &VoidCoinGUI::gotoOverviewPage);
    ...
    connect(receiveCoinsAction, &QAction::triggered, this, &VoidCoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &VoidCoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your VoidCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified VoidCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed VoidCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed VoidCoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a voidcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible VoidCoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &VoidCoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &VoidCoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &VoidCoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &VoidCoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &VoidCoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &VoidCoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &VoidCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &VoidCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &VoidCoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &VoidCoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &VoidCoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &VoidCoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &VoidCoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &VoidCoinGUI::showDebugWindow);
}

void VoidCoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void VoidCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &VoidCoinGUI::setCurrentWalletBySelectorIndex);
...
void VoidCoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &VoidCoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &VoidCoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &VoidCoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &VoidCoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void VoidCoinGUI::enableHistoryAction(bool privacy)
{
...
void VoidCoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &VoidCoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &VoidCoinGUI::removeWallet);
...
WalletController* VoidCoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void VoidCoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &VoidCoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &VoidCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &VoidCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &VoidCoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &VoidCoinGUI::incomingTransaction);
    connect(this, &VoidCoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void VoidCoinGUI::removeWallet(WalletModel* walletModel)
{
...
void VoidCoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void VoidCoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void VoidCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void VoidCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void VoidCoinGUI::createTrayIcon()
{
...
void VoidCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &VoidCoinGUI::toggleHidden);
...
void VoidCoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void VoidCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void VoidCoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void VoidCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void VoidCoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void VoidCoinGUI::openClicked()
{
...
void VoidCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void VoidCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void VoidCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void VoidCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void VoidCoinGUI::gotoVaultsPage()
{
    if (walletFrame) {
...
void VoidCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void VoidCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void VoidCoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void VoidCoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to VoidCoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void VoidCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void VoidCoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void VoidCoinGUI::updateHeadersSyncProgressLabel()
{
...
void VoidCoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void VoidCoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &VoidCoinGUI::quitRequested);
...
void VoidCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void VoidCoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &VoidCoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void VoidCoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void VoidCoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &VoidCoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &VoidCoinGUI::show);
                e->ignore();
            }
        }
    }
...
void VoidCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void VoidCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void VoidCoinGUI::incomingTransaction(const QString& date, VoidCoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(VoidCoinUnits::formatWithUnit(unit, amount, true));
...
void VoidCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void VoidCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool VoidCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool VoidCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void VoidCoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void VoidCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void VoidCoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void VoidCoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void VoidCoinGUI::updateWindowTitle()
{
...
void VoidCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void VoidCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void VoidCoinGUI::detectShutdown()
{
...
void VoidCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void VoidCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(VoidCoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = VoidCoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void VoidCoinGUI::subscribeToCoreSignals()
{
...
void VoidCoinGUI::unsubscribeFromCoreSignals()
{
...
bool VoidCoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<VoidCoinUnit> units = VoidCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const VoidCoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, VoidCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const VoidCoinUnit u : VoidCoinUnits::availableUnits()) {
        menu->addAction(VoidCoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(VoidCoinUnit newUnits)
{
    setText(VoidCoinUnits::longName(newUnits));
}
