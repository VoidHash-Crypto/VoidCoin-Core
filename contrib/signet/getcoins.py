...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing voidcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='voidcoin-cli', help='voidcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='VoidCoin address to which the faucet should send')
...
parser.add_argument('voidcoin_cli_args', nargs='*', help='Arguments to pass on to voidcoin-cli (default: -signet)')

args = parser.parse_args()

if args.voidcoin_cli_args == []:
    args.voidcoin_cli_args = ['-signet']


def voidcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.voidcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = voidcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = voidcoin_cli(['getnewaddress', 'faucet', 'bech32'])
