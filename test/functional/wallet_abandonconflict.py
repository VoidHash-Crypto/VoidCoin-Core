...
class AbandonConflictTest(VoidCoinTestFramework):
    ...
    def run_test(self):
        ...
        txAVOID = self.nodes[0].sendrawtransaction(signed2["hex"])

        # Create a child tx spending AVOID
        signed3_change = Decimal("24.999")
        inputs = [{"txid": txAVOID, "vout": 0}]
        ...
        assert not txAVOID in [utxo["txid"] for utxo in alice.listunspent(0)]
        balance = newbalance
        ...
        # Verify that B and C's 10 VOID outputs are available for spending again because AB1 is now conflicted
        assert_equal(alice.gettransaction(txAB1)["confirmations"], -1)
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance + Decimal("20"))
        balance = newbalance

        # Invalidate the block with the double spend. B & C's 10 VOID outputs should no longer be available
