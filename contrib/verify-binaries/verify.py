...
"""Script for verifying VoidCoin Core release binaries.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/voidcoin-core/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the VoidCoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"voidcoin_verify_binaries.{args.version}"
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the voidcoin release to download; of the format '
