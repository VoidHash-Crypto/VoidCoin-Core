...
static constexpr std::string_view VOID_V2_FRAGMENT_MSG_TYPE{"void_coinfrag"};

static constexpr size_t VOID_V2_MAX_BIP324_CONTENTS_LEN{0x00ffffff};
static constexpr size_t VOID_V2_FRAGMENT_DATA_SIZE{8'000'000};
static constexpr size_t VOID_V2_MAX_REASSEMBLED_MESSAGE_SIZE{MAX_PROTOCOL_MESSAGE_LENGTH};
static constexpr size_t VOID_V2_MAX_FRAGMENTS{
    (VOID_V2_MAX_REASSEMBLED_MESSAGE_SIZE + VOID_V2_FRAGMENT_DATA_SIZE - 1) /
    VOID_V2_FRAGMENT_DATA_SIZE
};

static_assert(VOID_V2_MAX_FRAGMENTS == 5);
static_assert(VOID_V2_FRAGMENT_DATA_SIZE < VOID_V2_MAX_BIP324_CONTENTS_LEN);
static_assert(VOID_V2_MAX_REASSEMBLED_MESSAGE_SIZE >= MAX_BLOCK_SERIALIZED_SIZE);
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        if (m_recv_len > MAX_CONTENTS_LEN) {
            ...
            LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: v2 packet too large len=%u max_contents=%u peer=%d\n",
                m_recv_len,
        ...
        if (!ignore) {
            ...
            case RecvState::APP:
            {
                // VoidCoin v2 fragmentation layer.
                //
                // BIP324 has a 3-byte encrypted packet length field, so a full 32 MB VoidCoin
                // block cannot be carried in one v2 packet. Large application messages are
                // carried as transport-internal "void_coinfrag" packets and reassembled here before
                // net_processing sees them.
                Span<const uint8_t> void_coin_contents{m_recv_decode_buffer};
                auto void_coin_msg_type = Kv5DecodeV2MessageType(void_coin_contents);

                if (void_coin_msg_type && *void_coin_msg_type == std::string{VOID_V2_FRAGMENT_MSG_TYPE}) {
                    Kv5Frag frag;
                    if (!Kv5ParseFragmentPayload(void_coin_contents, frag)) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: malformed fragment peer=%d\n", m_nodeid);
                        return false;
                    }

                    if (frag.original_type.empty() || frag.original_type == std::string{VOID_V2_FRAGMENT_MSG_TYPE}) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: invalid original type type=%s peer=%d\n",
                            frag.original_type,
                            m_nodeid);
                        return false;
                    }

                    if (frag.total_size == 0 || frag.total_size > VOID_V2_MAX_REASSEMBLED_MESSAGE_SIZE) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: bad total_size=%u max=%u peer=%d\n",
                            frag.total_size,
                            unsigned(VOID_V2_MAX_REASSEMBLED_MESSAGE_SIZE),
                            m_nodeid);
                        return false;
                    }

                    if (frag.fragment_count == 0 || frag.fragment_count > VOID_V2_MAX_FRAGMENTS) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: bad fragment_count=%u max=%u peer=%d\n",
                            frag.fragment_count,
                            unsigned(VOID_V2_MAX_FRAGMENTS),
                            m_nodeid);
                        return false;
                    }

                    if (frag.fragment_size == 0 || frag.fragment_size > VOID_V2_FRAGMENT_DATA_SIZE) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: bad fragment_size=%u max=%u peer=%d\n",
                            frag.fragment_size,
                            unsigned(VOID_V2_FRAGMENT_DATA_SIZE),
                            m_nodeid);
                        return false;
                    }

                    if (frag.fragment_index >= frag.fragment_count) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: index out of range index=%u count=%u peer=%d\n",
                            frag.fragment_index,
                            frag.fragment_count,
                            m_nodeid);
                        return false;
                    ...
                    const uint32_t expected_offset = uint32_t(size_t(frag.fragment_index) * VOID_V2_FRAGMENT_DATA_SIZE);

                    if (frag.fragment_offset != expected_offset) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: bad offset got=%u expected=%u index=%u peer=%d\n",
                    ...
                    if (uint64_t(frag.fragment_offset) + uint64_t(frag.fragment_size) > uint64_t(frag.total_size)) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: fragment exceeds total offset=%u size=%u total=%u peer=%d\n",
                            frag.fragment_offset,
                            frag.fragment_size,
                            frag.total_size,
                            m_nodeid);
                    ...
                    if (!is_last && frag.fragment_size != VOID_V2_FRAGMENT_DATA_SIZE) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: non-last fragment wrong size index=%u size=%u expected=%u peer=%d\n",
                            frag.fragment_index,
                            frag.fragment_size,
                            unsigned(VOID_V2_FRAGMENT_DATA_SIZE),
                            m_nodeid);
                    ...
                    if (!m_void_coin_v2_reassembly.active) {
                        if (frag.fragment_index != 0 || frag.fragment_offset != 0) {
                            LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: first fragment not index zero index=%u offset=%u peer=%d\n",
                                frag.fragment_index,
                                frag.fragment_offset,
                                m_nodeid);
                            return false;
                        ...
                        m_void_coin_v2_reassembly.active = true;
                        m_void_coin_v2_reassembly.original_type = frag.original_type;
                        m_void_coin_v2_reassembly.message_id = frag.message_id;
                        m_void_coin_v2_reassembly.total_size = frag.total_size;
                        m_void_coin_v2_reassembly.fragment_count = frag.fragment_count;
                        m_void_coin_v2_reassembly.next_fragment_index = 0;
                        m_void_coin_v2_reassembly.next_offset = 0;
                        m_void_coin_v2_reassembly.payload.clear();
                        m_void_coin_v2_reassembly.payload.reserve(frag.total_size);

                        LogPrintf("VOIDCOIN_V2_FRAGMENT: starting reassembly type=%s total=%u fragments=%u peer=%d\n",
                            frag.original_type,
                            frag.total_size,
                            frag.fragment_count,
                            m_nodeid);
                    ...
                    auto& r = m_void_coin_v2_reassembly;
                    ...
                    if (frag.message_id != r.message_id ||
                        ...
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: unexpected fragment id=%u index=%u expected_index=%u offset=%u expected_offset=%u peer=%d\n",
                    ...
                    if (r.next_offset != r.total_size || r.next_fragment_index != r.fragment_count) {
                        LogPrintf("VOIDCOIN_V2_FRAGMENT_ERROR: final size mismatch got=%u total=%u index=%u count=%u peer=%d\n",
                            r.next_offset,
                            r.total_size,
                            r.next_fragment_index,
                            r.fragment_count,
                    ...
                    LogPrintf("VOIDCOIN_V2_FRAGMENT: reassembled message type=%s total=%u fragments=%u peer=%d\n",
                        r.original_type,
                    ...
                    m_void_coin_v2_reassembly = Kv5V2ReassemblyState{};
                }
                ...
                // For a completed void_coinfrag sequence, this emits the reassembled original message.
...
bool V2Transport::SetMessageToSend(CSerializedNetMsg& msg) noexcept
{
    ...
    // application message. For fragmented VoidCoin v2 messages, all encrypted
    ...
    if (contents.size() <= VOID_V2_MAX_BIP324_CONTENTS_LEN) {
    ...
    // VoidCoin large-message path:
    // BIP324 uses a 3-byte packet length, so full 32 MB VoidCoin blocks must be
    // fragmented into multiple v2 packets and reassembled before net_processing.
    const size_t payload_size = msg.data.size();

    if (payload_size > VOID_V2_MAX_REASSEMBLED_MESSAGE_SIZE) {
        LogPrintf(
            "VOIDCOIN_V2_FRAGMENT_ERROR: refusing oversized message type=%s payload=%u max=%u peer=%d\n",
            msg.m_type,
    ...
    const uint16_t fragment_count = uint16_t(
        (payload_size + VOID_V2_FRAGMENT_DATA_SIZE - 1) / VOID_V2_FRAGMENT_DATA_SIZE
    );

    if (fragment_count == 0 || fragment_count > VOID_V2_MAX_FRAGMENTS) {
        LogPrintf(
            "VOIDCOIN_V2_FRAGMENT_ERROR: invalid fragment count type=%s payload=%u fragments=%u max=%u peer=%d\n",
            msg.m_type,
            unsigned(payload_size),
    ...
    const uint64_t message_id = ++m_void_coin_v2_send_fragment_id;

    LogPrintf(
        "VOIDCOIN_V2_FRAGMENT: sending fragmented message type=%s payload=%u fragments=%u chunk=%u peer=%d\n",
        msg.m_type,
        unsigned(payload_size),
        unsigned(fragment_count),
        unsigned(VOID_V2_FRAGMENT_DATA_SIZE),
    ...
    for (uint16_t index = 0; index < fragment_count; ++index) {
        const uint32_t offset = uint32_t(size_t(index) * VOID_V2_FRAGMENT_DATA_SIZE);
        ...
        const auto frag_contents = Kv5BuildV2LongContents(
            VOID_V2_FRAGMENT_MSG_TYPE,
            Span<const uint8_t>{frag_payload}
        );

        if (frag_contents.size() > VOID_V2_MAX_BIP324_CONTENTS_LEN) {
            LogPrintf(
                "VOIDCOIN_V2_FRAGMENT_ERROR: fragment contents too large type=%s index=%u contents=%u max=%u peer=%d\n",
                msg.m_type,
                unsigned(index),
                unsigned(frag_contents.size()),
                unsigned(VOID_V2_MAX_BIP324_CONTENTS_LEN),
                m_nodeid);
            ClearShrink(m_send_buffer);
            return false;
        }
