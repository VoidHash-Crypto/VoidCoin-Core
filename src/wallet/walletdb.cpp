...
const std::string VOIDCOIN_P2QR_KEY{"voidcoinp2qrkey"};
const std::string VOIDCOIN_P2QR_MULTISIG{"voidcoinp2qrmultisig"};
...
bool WalletBatch::WriteVoidCoinP2QRKey(const uint256& key_hash, const P2QRKeyRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::VOIDCOIN_P2QR_KEY, key_hash), record, false);
}

bool WalletBatch::WriteVoidCoinP2QRMultisig(const uint256& program, const VoidCoinP2QRMultisigRecord& record)
{
    return WriteIC(std::make_pair(DBKeys::VOIDCOIN_P2QR_MULTISIG, program), record, false);
}
...
static DBErrors LoadLegacyWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    // Load native VOID P2QR seed-backed keys.
    // This is intentionally loaded for both legacy and descriptor wallets.
    LoadResult voidcoin_p2qr_key_res = LoadRecords(pwallet, batch, DBKeys::VOIDCOIN_P2QR_KEY,
        ...
        if (!pwallet->LoadVoidCoinP2QRKey(key_hash, record)) {
            strErr = "Error reading wallet database: CWallet::LoadVoidCoinP2QRKey failed";
            return DBErrors::NONCRITICAL_ERROR;
        }

        return DBErrors::LOAD_OK;
    });
    result = std::max(result, voidcoin_p2qr_key_res.m_result);
    
    LoadResult voidcoin_p2qr_multisig_res = LoadRecords(pwallet, batch, DBKeys::VOIDCOIN_P2QR_MULTISIG,
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& strErr) {
        uint256 program;
        key >> program;
        ...
        if (!pwallet->LoadVoidCoinP2QRMultisig(program, record)) {
            strErr = "Error reading wallet database: CWallet::LoadVoidCoinP2QRMultisig failed";
            return DBErrors::NONCRITICAL_ERROR;
        }

        return DBErrors::LOAD_OK;
    });
    result = std::max(result, voidcoin_p2qr_multisig_res.m_result);
    ...
    LoadResult keymeta_res = LoadRecords(pwallet, batch, DBKeys::KEYMETA,
        ...
        if (keyMeta.nVersion >= CKeyMetadata::VERSION_WITH_HDDATA && !keyMeta.hd_seed_id.IsNull() && keyMeta.hdKeypath.size() > 0) {
            ...
            // See https://github.com/voidcoin/voidcoin/pull/12924
            bool internal = false;
            uint32_t index = 0;
