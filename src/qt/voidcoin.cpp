...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<VoidCoinUnit>("VoidCoinUnit");
#else
    qRegisterMetaType<VoidCoinUnit>("VoidCoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. voidcoin_de.qm (shortcut "de" needs to be defined in voidcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. voidcoin_de_DE.qm (shortcut "de_DE" needs to be defined in voidcoin.qrc)
...
static const char* qt_argv = "voidcoin-qt";

VoidCoinApplication::VoidCoinApplication()
...
void VoidCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the VoidCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", VoidCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void VoidCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool VoidCoinApplication::createOptionsModel(bool resetSettings)
{
...
void VoidCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new VoidCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &VoidCoinGUI::quitRequested, this, &VoidCoinApplication::requestShutdown);
...
void VoidCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void VoidCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool VoidCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void VoidCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &VoidCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &VoidCoinApplication::handleRunawayException);
    connect(this, &VoidCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &VoidCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void VoidCoinApplication::parameterSetup()
{
...
void VoidCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void VoidCoinApplication::requestInitialize()
{
...
void VoidCoinApplication::requestShutdown()
{
...
void VoidCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // voidcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &VoidCoinGUI::handlePaymentRequest);
            connect(window, &VoidCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void VoidCoinApplication::handleRunawayException(const QString &message)
{
...
void VoidCoinApplication::handleNonFatalException(const QString& message)
{
...
WId VoidCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool VoidCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", VoidCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(voidcoin);
    Q_INIT_RESOURCE(voidcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(VOIDCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see voidcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see voidcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse voidcoin.conf, determine network, switch to network specific
    ...
    // voidcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
