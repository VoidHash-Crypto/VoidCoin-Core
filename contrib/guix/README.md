# Bootstrappable VoidCoin Core Builds

This directory contains the files necessary to perform bootstrappable VoidCoin
Core builds.
...
To build VoidCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      VoidCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/voidcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/voidcoin-core/guix.sigs).
