...
TMPDIR_PREFIX = "voidcoin_func_test_"
...
class VoidCoinTestMetaClass(type):
    """Metaclass for VoidCoinTestFramework.

    Ensures that any attempt to register a subclass of `VoidCoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'VoidCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("VoidCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("VoidCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class VoidCoinTestFramework(metaclass=VoidCoinTestMetaClass):
    """Base class for a voidcoin test script.

    Individual voidcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave voidcoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use voidcoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "voidcoind": ("voidcoind", "VOIDCOIND"),
            "voidcoin-cli": ("voidcoincli", "VOIDCOINCLI"),
            "voidcoin-util": ("voidcoinutil", "VOIDCOINUTIL"),
            "voidcoin-wallet": ("voidcoinwallet", "VOIDCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("VoidCoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'voidcoind', self.options.voidcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'voidcoin-cli', self.options.voidcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a voidcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple voidcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the voidcoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as voidcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    voidcoind=self.options.voidcoind,
                    voidcoin_cli=self.options.voidcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in voidcoin.conf
    ...
    def skip_if_no_voidcoind_tracepoints(self):
        """Skip the running test if voidcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("voidcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_voidcoind_zmq(self):
        """Skip the running test if voidcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("voidcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if voidcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("voidcoin-wallet has not been compiled")

    def skip_if_no_voidcoin_util(self):
        """Skip the running test if voidcoin-util has not been compiled."""
        if not self.is_voidcoin_util_compiled():
            raise SkipTest("voidcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if voidcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("voidcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether voidcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether voidcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_voidcoin_util_compiled(self):
        """Checks whether voidcoin-util was compiled."""
