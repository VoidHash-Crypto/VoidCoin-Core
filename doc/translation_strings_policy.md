...
This document provides guidelines for internationalization of the VoidCoin Core software.
...
This can be checked by building the `translate` target with `cmake` ([instructions](translation_process.md)), then verifying that `voidcoin_en.ts` remains unchanged.
