UNIX BUILD NOTES
====================
Some notes on how to build VoidCoin Core in Unix.
...
memory available when compiling VoidCoin Core. On systems with less, gcc can be
...
To build VoidCoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/voidcoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/voidcoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, VoidCoin Core can
