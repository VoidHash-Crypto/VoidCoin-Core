...
#ifndef VOIDCOIN_NET_H
#define VOIDCOIN_NET_H
...
/*
 * VoidCoin protocol message size limit.
 *
 * VoidCoin supports larger serialized blocks than Bitcoin. This value must
...
private:
    ...
    // VoidCoin v2 transport fragmentation.
    //
    // BIP324 uses a 3-byte encrypted packet length field, so one encrypted v2
    // packet cannot carry a full 32 MB VoidCoin block. Large application messages
    // are split into void_coinfrag transport-internal messages and reassembled before
    ...
    Kv5V2ReassemblyState m_void_coin_v2_reassembly GUARDED_BY(m_recv_mutex);
    uint64_t m_void_coin_v2_send_fragment_id GUARDED_BY(m_send_mutex){0};
...
public:
     ...
     * @see https://github.com/voidcoin/voidcoin/issues/21744 for details.
     */
