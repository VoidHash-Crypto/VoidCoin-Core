...
static CMutableTransaction BuildTransactionTestCase(uint8_t output_tag = 0x01)
{
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = 42;
    tx.vout[0].scriptPubKey = VoidCoinTestP2QRScript(output_tag);
    return tx;
}
...
static CBlock BuildBlockTestCase(FastRandomContext& ctx)
{
    ...
    tx.vout[0].scriptPubKey = VoidCoinTestP2QRScript(0x21);
    ...
    tx.vout[0].scriptPubKey = VoidCoinTestP2QRScript(0x41);
