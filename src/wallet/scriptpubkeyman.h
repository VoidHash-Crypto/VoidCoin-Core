...
#ifndef VOIDCOIN_WALLET_SCRIPTPUBKEYMAN_H
#define VOIDCOIN_WALLET_SCRIPTPUBKEYMAN_H
 ...
 * change.
 *
 * The VoidCoin Core wallet was originally a collection of unrelated private
...
class VoidCoinP2QRScriptPubKeyMan : public ScriptPubKeyMan
{
private:
    mutable Mutex m_mutex;
    std::map<uint256, P2QRKeyRecord> m_keys GUARDED_BY(m_mutex);
    std::map<uint256, VoidCoinP2QRMultisigRecord> m_multisig GUARDED_BY(m_mutex);

    static uint256 KeyHashFromDestination(const VoidCoinP2QRDestination& dest)
    {
        uint256 key_hash;
...
public:
    explicit VoidCoinP2QRScriptPubKeyMan(WalletStorage& storage) : ScriptPubKeyMan(storage) {}

    util::Result<CTxDestination> GetNewDestination(const OutputType type) override;
    isminetype IsMine(const CScript& script) const override;
    bool GetDestinationForP2SHScriptID(const CScriptID& script_id, VoidCoinP2QRDestination& dest) const;
    bool CanGetAddresses(bool internal = false) const override { return !internal; }
    bool HavePrivateKeys() const override { return true; }
    uint256 GetID() const override { return Hash(std::string{"void_coin-p2qr-spkm"}); }
    ...
    bool ImportSeed(Span<const unsigned char> seed, VoidCoinP2QRDestination& dest);
    bool ExportSeed(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& seed_out) const;
    bool GetKey(const VoidCoinP2QRDestination& dest, CPQKey& key_out) const;
    bool GetPubKeyForDestination(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& pubkey_out) const;
    bool HaveDestination(const VoidCoinP2QRDestination& dest) const;

    bool AddMultisigPolicy(const uint256& program, const VoidCoinP2QRMultisigRecord& record);
    bool LoadMultisigPolicy(const uint256& program, const VoidCoinP2QRMultisigRecord& record);
    bool HaveMultisigProgram(const uint256& program) const;
    bool GetMultisigRecord(const uint256& program, VoidCoinP2QRMultisigRecord& record_out) const;
    bool GetMultisigProgramForP2SHScriptID(const CScriptID& script_id, uint256& program_out) const;
    std::vector<std::pair<uint256, VoidCoinP2QRMultisigRecord>> ListMultisigPolicies() const;
...
#endif // VOIDCOIN_WALLET_SCRIPTPUBKEYMAN_H
