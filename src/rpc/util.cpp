...
CFeeRate ParseFeeRate(const UniValue& json)
{
    ...
    if (val >= COIN) throw JSONRPCError(RPC_INVALID_PARAMETER, "Fee rates larger than or equal to 1VOID/kvB are not accepted");
    return CFeeRate{val};
}
...
{
    return "> voidcoin-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> voidcoin-cli -named " + methodname;
...
public:
    ...
    UniValue operator()(const VoidCoinP2QRDestination& id) const
    {
       ...
       const CScript mining_script = consensus::voidcoinp2qr::MakeP2SHScriptPubKey(program);
       const CScriptID mining_script_id(redeem_script);
       const CTxDestination mining_dest{ScriptHash(mining_script_id)};

       obj.pushKV("isscript", false);
       obj.pushKV("iswitness", false);
       obj.pushKV("isvoidcoinp2qr", true);
