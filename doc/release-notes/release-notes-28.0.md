VoidCoin Core version 28.0 is now available from:

  <https://bitcoincore.org/bin/voidcoin-core-28.0/>
...
installer (on Windows) or just copy over `/Applications/VoidCoin-Qt` (on macOS)
...
Running VoidCoin Core binaries on macOS requires self signing.
```
cd /path/to/voidcoin-28.0/bin
xattr -d com.apple.quarantine voidcoin-cli voidcoin-qt voidcoin-tx voidcoin-util voidcoin-wallet voidcoind test_voidcoin
codesign -s - voidcoin-cli voidcoin-qt voidcoin-tx voidcoin-util voidcoin-wallet voidcoind test_voidcoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. VoidCoin
...
Support for Testnet4 as specified in [BIP94](https://github.com/voidcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\VoidCoin`
to `C:\Users\Username\AppData\Local\VoidCoin`. VoidCoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/voidcoin/voidcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libvoidcoinconsensus Removal
---------------------------

The libvoidcoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if VoidCoin Core was listening for P2P connections, either using
...
- VoidCoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/voidcoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running VoidCoin Core with the configuration
...
are now required to compile VoidCoin Core. (#29091, #30263)

- The minimum required glibc to run VoidCoin Core is now
...
- A new `-testdatadir` option has been added to `test_voidcoin` to allow specifying the
...
Previous releases of VoidCoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
