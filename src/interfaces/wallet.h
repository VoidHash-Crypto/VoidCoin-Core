...
#ifndef VOIDCOIN_INTERFACES_WALLET_H
#define VOIDCOIN_INTERFACES_WALLET_H
...
struct VoidCoinP2QRMultisigInfo
{
    std::string address;
    std::string program;
...
struct VoidCoinVaultSignerInfo
{
    int signer_index{0};
    std::string pubkey;
    std::string address;
...
struct VoidCoinVaultInfo
{
    ...
    int utxos{0};
    int spend_count{0};
    std::vector<VoidCoinVaultSignerInfo> signers;
};
...
public:
    ...
    //! Generate a new local VoidCoin P2QR signer key and return its public identity.
    virtual util::Result<VoidCoinP2QRSignerInfo> createVoidCoinP2QRSigner(
        const std::string& label) = 0;
    
    //! Create and store a wallet-owned VoidCoin P2QR multisig policy.
    virtual util::Result<VoidCoinP2QRMultisigInfo> createVoidCoinP2QRMultisig(
        int required,
        const std::vector<std::string>& signer_addresses,
        const std::string& label,
        bool include_wrapped) = 0;
    
    //! Create a normal-user VoidCoin vault payment package JSON.
    virtual util::Result<std::string> createVoidCoinVaultTxPackage(
        const std::vector<wallet::CRecipient>& recipients,
        const wallet::CCoinControl& coin_control) = 0;

    //! Approve/sign a normal-user VoidCoin vault payment package JSON.
    virtual util::Result<std::string> approveVoidCoinVaultTxPackage(
        const std::string& package_json) = 0;

    //! Finalize a complete VoidCoin vault payment package and return transaction hex.
    virtual util::Result<std::string> finalizeVoidCoinVaultTxPackage(
        const std::string& package_json) = 0;
    
    //! Write a VoidCoin P2QR multisig vault backup package to a local file.
    virtual util::Result<std::string> dumpVoidCoinVaultBackup(
        const std::string& address,
        const std::string& filename) = 0;

    //! Restore a VoidCoin P2QR multisig vault backup package from a local file.
    virtual util::Result<std::string> importVoidCoinVaultBackup(
        const std::string& filename,
        const std::string& label) = 0;
    
    
    //! VoidCoin MuSig Vault and Vault UTXO Management.
    virtual std::vector<VoidCoinVaultUTXO> listVoidCoinVaultUTXOs() = 0;
    virtual std::vector<VoidCoinVaultInfo> listVoidCoinVaults() = 0;
...
#endif // VOIDCOIN_INTERFACES_WALLET_H
