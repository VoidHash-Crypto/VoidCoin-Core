...
util::Result<CTxDestination> VoidCoinP2QRScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    CPQKey key;
    if (!key.MakeNewKey()) {
        return util::Error{_("Error: Failed to generate VOID P2QR key.")};
    }
    ...
    if (!AddKey(batch, key_hash, record)) {
        return util::Error{_("Error: Failed to write VOID P2QR key to wallet database.")};
    }

    return CTxDestination{dest};
}

isminetype VoidCoinP2QRScriptPubKeyMan::IsMine(const CScript& script) const
{
    uint256 program;
    ...
    if (consensus::voidcoinp2qr::IsP2SHOutput(script, script_id)) {
        ...
        for (const auto& [key_hash, record] : m_keys) {
            const CScript redeem_script = consensus::voidcoinp2qr::MakeRedeemScript(key_hash);
        ...
        for (const auto& [msig_program, record] : m_multisig) {
            const CScript redeem_script = consensus::voidcoinp2qr::MakeRedeemScript(msig_program);
            if (CScriptID(redeem_script) == script_id) {
                return ISMINE_SPENDABLE;
            }
        }
...
std::unordered_set<CScript, SaltedSipHasher> VoidCoinP2QRScriptPubKeyMan::GetScriptPubKeys() const
{
    ...
    for (const auto& [key_hash, record] : m_keys) {
        const VoidCoinP2QRDestination dest{key_hash};
        ...
        // P2SH-carried P2QR compatibility container:
        //   a9 14 HASH160(50 20 <program>) 87
        scripts.insert(consensus::voidcoinp2qr::MakeP2SHScriptPubKey(key_hash));
    }

    for (const auto& [msig_program, record] : m_multisig) {
        const VoidCoinP2QRDestination dest{msig_program};
        ...
        scripts.insert(consensus::voidcoinp2qr::MakeP2SHScriptPubKey(msig_program));
    }
...
bool VoidCoinP2QRScriptPubKeyMan::CanProvide(const CScript& script, SignatureData& sigdata)
{
    return IsMine(script) != ISMINE_NO;
}

bool VoidCoinP2QRScriptPubKeyMan::LoadKey(const uint256& key_hash, const P2QRKeyRecord& record)
{
...
bool VoidCoinP2QRScriptPubKeyMan::AddKey(WalletBatch& batch, const uint256& key_hash, const P2QRKeyRecord& record)
{
    if (!LoadKey(key_hash, record)) {
        return false;
    }

    return batch.WriteVoidCoinP2QRKey(key_hash, record);
}

bool VoidCoinP2QRScriptPubKeyMan::ImportSeed(Span<const unsigned char> seed, VoidCoinP2QRDestination& dest)
{
    CPQKey key;
...
bool VoidCoinP2QRScriptPubKeyMan::ExportSeed(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& seed_out) const
{
...
bool VoidCoinP2QRScriptPubKeyMan::GetKey(const VoidCoinP2QRDestination& dest, CPQKey& key_out) const
{
    const uint256 key_hash = KeyHashFromDestination(dest);
...
bool VoidCoinP2QRScriptPubKeyMan::HaveDestination(const VoidCoinP2QRDestination& dest) const
{
    const uint256 key_hash = KeyHashFromDestination(dest);
...
bool VoidCoinP2QRScriptPubKeyMan::GetPubKeyForDestination(const VoidCoinP2QRDestination& dest, std::vector<unsigned char>& pubkey_out) const
{
    const uint256 key_hash = KeyHashFromDestination(dest);
...
bool VoidCoinP2QRScriptPubKeyMan::AddMultisigPolicy(const uint256& program, const VoidCoinP2QRMultisigRecord& record)
{
    std::string error;
    if (!VoidCoinIsCanonicalP2QRMultisigPolicy(record.required, record.signer_programs, &error)) {
        WalletLogPrintf("VOIDCOIN_P2QR multisig policy rejected: %s\n", error);
        return false;
    }

    const uint256 expected_program = VoidCoinP2QRMultisigProgram(record.required, record.signer_programs);
    if (expected_program != program) {
        WalletLogPrintf("VOIDCOIN_P2QR multisig policy rejected: program mismatch\n");
        return false;
    }
    ...
    WalletBatch batch(m_storage.GetDatabase());
    return batch.WriteVoidCoinP2QRMultisig(program, record);
}

bool VoidCoinP2QRScriptPubKeyMan::LoadMultisigPolicy(const uint256& program, const VoidCoinP2QRMultisigRecord& record)
{
    std::string error;
    if (!VoidCoinIsCanonicalP2QRMultisigPolicy(record.required, record.signer_programs, &error)) {
        WalletLogPrintf("VOIDCOIN_P2QR multisig policy load rejected: %s\n", error);
        return false;
    }

    const uint256 expected_program = VoidCoinP2QRMultisigProgram(record.required, record.signer_programs);
    if (expected_program != program) {
        WalletLogPrintf("VOIDCOIN_P2QR multisig policy load rejected: program mismatch\n");
        return false;
    }
...
bool VoidCoinP2QRScriptPubKeyMan::HaveMultisigProgram(const uint256& program) const
{
    LOCK(m_mutex);
    return m_multisig.count(program) > 0;
}

bool VoidCoinP2QRScriptPubKeyMan::GetMultisigRecord(const uint256& program, VoidCoinP2QRMultisigRecord& record_out) const
{
    LOCK(m_mutex);
...
bool VoidCoinP2QRScriptPubKeyMan::GetMultisigProgramForP2SHScriptID(const CScriptID& script_id, uint256& program_out) const
{
    LOCK(m_mutex);
...
std::vector<std::pair<uint256, VoidCoinP2QRMultisigRecord>>
VoidCoinP2QRScriptPubKeyMan::ListMultisigPolicies() const
{
    std::vector<std::pair<uint256, VoidCoinP2QRMultisigRecord>> result;
...
bool VoidCoinP2QRScriptPubKeyMan::GetKeyByPubKey(const std::vector<unsigned char>& pubkey, CPQKey& key_out) const
{
...
bool VoidCoinP2QRScriptPubKeyMan::SignTransaction(
    ...
    std::map<int, bilingual_str>& input_errors) const
{
    ...
    // Native/P2SH-carried VOIDCOIN_P2QR v1 supports SIGHASH_DEFAULT/ALL only.
    ...
    for (uint32_t i = 0; i < tx.vin.size(); ++i) {
        ...
        if (consensus::voidcoinp2qr::IsOutput(spent_output.scriptPubKey, program)) {
            native_p2qr = true;
            p2qr_redeem_script = consensus::voidcoinp2qr::MakeRedeemScript(program);
        } else {
            CScriptID spent_script_id;
            if (!consensus::voidcoinp2qr::IsP2SHOutput(spent_output.scriptPubKey, spent_script_id)) {
                continue;
            }
            ...
            // First try single-key P2SH-carried P2QR.
            {
                ...
                for (const auto& [candidate_key_hash, record] : m_keys) {
                    const CScript candidate_redeem_script = consensus::voidcoinp2qr::MakeRedeemScript(candidate_key_hash);
        ...
        if (is_multisig) {
            ...
            if (!VoidCoinIsCanonicalP2QRMultisigPolicy(
                ...
                input_errors[i] = Untranslated(strprintf(
                    "VOIDCOIN_P2QR multisig policy invalid: %s",
                    policy_error));
                return false;
            }

            const uint256 expected_program =
                VoidCoinP2QRMultisigProgram(multisig_record.required, multisig_record.signer_programs);
            if (expected_program != program) {
                input_errors[i] = Untranslated("VOIDCOIN_P2QR multisig policy program mismatch");
                return false;
            }
            ...
            for (uint16_t signer_index = 0;
                ...
                if (derived_signer_program != signer_program) {
                    input_errors[i] = Untranslated(
                        "VOIDCOIN_P2QR multisig local pubkey does not match signer address policy");
                    return false;
                }
                ...
                if (!key.Sign(Span<const unsigned char>(msg_hash.begin(), msg_hash.size()), signature)) {
                    input_errors[i] = Untranslated("VOIDCOIN_P2QR multisig ML-DSA-87 signing failed");
                    return false;
                }

                if (signature.empty()) {
                    input_errors[i] = Untranslated("VOIDCOIN_P2QR multisig ML-DSA-87 signing returned empty signature");
                    return false;
                }
            ...
            if (signatures.size() < multisig_record.required) {
                input_errors[i] = Untranslated(strprintf(
                    "VOIDCOIN_P2QR multisig private keys not found: have %u, need %u",
            ...
            for (const uint16_t signer_index : signer_indexes) {
                txin.scriptSig << VoidCoinP2QRUInt16Push(signer_index);
            }
            ...
            txin.scriptSig << VoidCoinP2QRUInt16Push(multisig_record.required);
            txin.scriptSig << VoidCoinP2QRUInt16Push(static_cast<uint16_t>(multisig_record.signer_programs.size()));
            ...
            CPQKey key;
            if (!GetKey(dest, key)) {
                input_errors[i] = Untranslated("VOIDCOIN_P2QR private key not found");
                return false;
            }

            std::vector<unsigned char> signature;
            if (!key.Sign(Span<const unsigned char>(msg_hash.begin(), msg_hash.size()), signature)) {
                input_errors[i] = Untranslated("VOIDCOIN_P2QR ML-DSA-87 signing failed");
                return false;
            }

            if (signature.empty()) {
                input_errors[i] = Untranslated("VOIDCOIN_P2QR ML-DSA-87 signing returned empty signature");
                return false;
            }
    ...
    // This SPKM's responsibility is only native/P2SH-carried VOIDCOIN_P2QR inputs.
...
bool VoidCoinP2QRScriptPubKeyMan::GetDestinationForP2SHScriptID(const CScriptID& script_id, VoidCoinP2QRDestination& dest) const
{
    LOCK(m_mutex);
    ...
    for (const auto& [key_hash, record] : m_keys) {
        const CScript redeem_script = consensus::voidcoinp2qr::MakeRedeemScript(key_hash);
        if (CScriptID(redeem_script) == script_id) {
            dest = VoidCoinP2QRDestination{key_hash};
            return true;
        }
    }
