...
This guide describes how to build voidcoind, command-line utilities, and GUI on FreeBSD.
...
### 2. Clone VoidCoin Repo
Now that `git` and all the required dependencies are installed, let's clone the VoidCoin Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `voidcoind` or `voidcoin-qt`.
...
```
to: /path/to/voidcoin/depends/x86_64-unknown-freebsd[release-number]
```
...
VoidCoin Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
