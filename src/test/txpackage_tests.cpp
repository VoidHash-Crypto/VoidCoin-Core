...
BOOST_AUTO_TEST_CASE(package_validation_tests)
{
    ...
    // legacy ECDSA/P2PKH path. VoidCoin's valid spend path is P2QR/ML-DSA, so
    // that helper asserts before package validation behavior is tested.
    //
    // Proper replacement: add a VoidCoin-native CreateValidTransaction helper
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    // Now that the child's fees have "increased" by 1 VOID, the cheap package should succeed.
    {
    ...
    // Parent pays 1 VOID and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
