...
VoidCoin Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/voidcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `VOIDCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from VoidCoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since VoidCoin Core version 0.18.0.
...
- Testing or debugging of implementations of the VoidCoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of VoidCoin Core. VoidCoin Core logs debug messages
...
- OpenSSL is no longer used by VoidCoin Core.  (#17265)

- BIP70 support has been fully removed from VoidCoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/voidcoin/voidcoin/issues/16599),
  [PR #16702](https://github.com/voidcoin/voidcoin/pull/16702), and the
  `voidcoind help` for more information.  This option is experimental and
...
- All custom settings configured when VoidCoin Core starts are now
...
- The "Start VoidCoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple VoidCoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed VoidCoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/voidcoin-core/HWI) for
  signing.  Future versions of VoidCoin Core should support a GUI option
...
- VoidCoin Core's automatically-generated source code documentation is
...
- The `voidcoin-cli` utility used with the `-getinfo` parameter now
...
- The `voidcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `voidcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_voidcoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_voidcoin` binary (hebasto)
...
- #18702 Fix ASLR for voidcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate voidcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest voidcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_voidcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (voidcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone voidcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about VoidCoin Core files (hebasto)
...
- #17119 Fix broken voidcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from voidcoin-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to voidcoin-wallet (hel-o)
...
- #18341 Replace remaining literal VOID with `CURRENCY_UNIT` (domob1812)
...
- #16115 On voidcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from voidcoin-cli -getinfo (malevolent)
