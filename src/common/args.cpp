...
const char * const VOIDCOIN_CONF_FILENAME = "voidcoin.conf";
const char * const VOIDCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //voidcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", VOIDCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\VoidCoin
    //   new: C:\Users\Username\AppData\Local\VoidCoin
    // macOS: ~/Library/Application Support/VoidCoin
    // Unix-like: ~/.voidcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "VoidCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "VoidCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/VoidCoin";
#else
    // Unix-like
    return pathRet / ".voidcoin";
#endif
#endif
}
