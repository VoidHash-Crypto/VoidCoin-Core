...
#include <qt/voidcoinaddressvalidator.h>
...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::MAIN:
        addr = "vqr1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tq2jku9f";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a VoidCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new VoidCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new VoidCoinAddressCheckValidator(parent));
}
...
bool parseVoidCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no voidcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("voidcoin"))
        return false;
...
bool parseVoidCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseVoidCoinURI(uriInstance, out);
}

QString formatVoidCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("voidcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openVoidCoinConf()
{
    ...
    /* Open voidcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "VoidCoin.lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("VoidCoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for VoidCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "voidcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("voidcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a voidcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=VoidCoin\n";
        else
            optionFile << strprintf("Name=VoidCoin (%s)\n", ChainTypeToString(chain));
