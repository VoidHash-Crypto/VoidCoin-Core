...
static bool VoidCoinRelayReadPushesOnly(
...
static bool VoidCoinRelayIsP2QRRedeemScript(const std::vector<unsigned char>& raw_script)
{
    ...
    return type == TxoutType::VOIDCOIN_P2QR &&
           solutions.size() == 1 &&
           solutions[0].size() == VOIDCOIN_P2QR_PROGRAM_SIZE;
}

static bool VoidCoinRelayIsSingleSigP2QRSpendPushes(
    const std::vector<std::vector<unsigned char>>& pushes)
{
    ...
    return consensus::voidcoinp2qr::ParseSpendData(native_spend_script, sig, pubkey, error);
}

static bool VoidCoinRelayIsMultisigP2QRSpendPushes(
    const std::vector<std::vector<unsigned char>>& pushes)
{
    VoidCoinP2QRMultisigSpend spend;
    std::string error;

    return VoidCoinParseP2QRMultisigSpend(pushes, spend, &error);
}

static bool IsVoidCoinP2QRRelaySpendEnvelope(const CScript& script_sig)
{
    std::vector<std::vector<unsigned char>> pushes;
    if (!VoidCoinRelayReadPushesOnly(script_sig, pushes)) {
        return false;
    }
     ...
     *   <sig> <pubkey>
     */
    if (VoidCoinRelayIsSingleSigP2QRSpendPushes(pushes)) {
        return true;
    }
     ...
     *   <m> <n>
     */
    if (VoidCoinRelayIsMultisigP2QRSpendPushes(pushes)) {
        return true;
    }
     ...
     *   50 20 <program>
     */
    if (pushes.size() == 3 && VoidCoinRelayIsP2QRRedeemScript(pushes.back())) {
        std::vector<std::vector<unsigned char>> inner_pushes{pushes.begin(), pushes.end() - 1};
        if (VoidCoinRelayIsSingleSigP2QRSpendPushes(inner_pushes)) {
            return true;
        }
    }
    ...
    if (pushes.size() > 3 && VoidCoinRelayIsP2QRRedeemScript(pushes.back())) {
        std::vector<std::vector<unsigned char>> inner_pushes{pushes.begin(), pushes.end() - 1};
        if (VoidCoinRelayIsMultisigP2QRSpendPushes(inner_pushes)) {
            return true;
        }
    }
...
static int32_t GetVoidCoinP2QRRelayFeeVSize(const TxMempoolInfo& txinfo)
{
    ...
    for (const CTxIn& txin : txinfo.tx->vin) {
        ...
        if (!IsVoidCoinP2QRRelaySpendEnvelope(txin.scriptSig)) {
            continue;
        }
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
                ...
                if (fSendTrickle && tx_relay->m_send_mempool) {
                    ...
                    for (const auto& txinfo : vtxinfo) {
                        ...
                        if (txinfo.fee < filterrate.GetFee(GetVoidCoinP2QRRelayFeeVSize(txinfo))) {
                            continue;
                        }
                        if (tx_relay->m_bloom_filter) {
