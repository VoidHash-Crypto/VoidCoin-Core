# User-space, Statically Defined Tracing (USDT) for VoidCoin Core

VoidCoin Core includes statically defined tracepoints to allow for more
...
**Note**: VoidCoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to VoidCoin Core
...
Multiple tools can list the available tracepoints in a `voidcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in VoidCoin Core, use `info probes` in `gdb`:

```
$ gdb ./build/bin/voidcoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/voidcoind
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/voidcoind
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/voidcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in VoidCoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/voidcoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/bin/voidcoind -v
